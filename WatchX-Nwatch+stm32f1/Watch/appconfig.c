...
void appconfig_init()
{
    ...
    if(BKPCheck == EEPROM_CHECK_NUM) // 已经设置过备份域
    {
        // 从备份域读数据
        int index = 0;
        appConfig.sleepTimeout = BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
        appConfig.invert       = BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
        #if COMPILE_ANIMATIONS
        appConfig.animations  =  BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
        #endif
        appConfig.display180  =  BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
        appConfig.CTRL_LEDs   =  BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
        appConfig.showFPS     =  BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
        appConfig.timeMode    = (timemode_t) BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
        appConfig.volumes     =  BKP_ReadBackupRegister(BKPDataRR[index]);
        index++;
    }
    else
    {
        ...
        int index = 0;
        BKP_WriteBackupRegister(BKPDataRR[index], BKPbuf[index]);
        index++;
        BKP_WriteBackupRegister(BKPDataRR[index], BKPbuf[index]);
        index++;
        #if COMPILE_ANIMATIONS
